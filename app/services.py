...
def add_budget_item(
    ...
    reference_id, descricao, quantidade, valor_unitario, desconto, total = _build_item_values(
        item_type=item_type,
    ...
    for payload in items_payload:
        created_items.append(
            add_work_order_item(
                ...
                desconto=payload.get('desconto', 0),
...
def create_work_order_from_budget(budget: Budget) -> WorkOrder:
    ...
    order = WorkOrder(
        ...
        placa=budget.placa,
        veiculo_descricao=budget.veiculo_descricao,
...
def ensure_work_order_receivables(order: WorkOrder) -> list[FinancialEntry]:
    ...
    existing = (
        FinancialEntry.query.filter_by(reference_type='OS', reference_id=order.id)
        .order_by(FinancialEntry.installment_number.asc(), FinancialEntry.id.asc())
        .all()
    )
    if existing:
        return existing
        ...
        if normalized == 'RECEBIDO':
            if entry.status == 'RECEBIDO':
                ...
                settle_financial_entry(
                    ...
                    payment_receipt_at=payment_receipt_at,
...
def services_rank(limit: int = 5):
    return (
        db.session.query(
            WorkOrderItem.descricao.label('descricao'),
