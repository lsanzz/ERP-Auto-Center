...
def _meta_table(order, styles: dict[str, ParagraphStyle]) -> Table:
    ...
    table.setStyle(TableStyle([
        ...
        ('ROWBACKGROUNDS', (0, 0), (-1, -1), [colors.whitesmoke, colors.HexColor('#f8f8f8')]),
...
def _items_table(title: str, items: list, empty_label: str, styles: dict[str, ParagraphStyle]) -> list:
    ...
    table.setStyle(TableStyle([
        ...
        ('BOX', (0, 0), (-1, -1), 0.7, colors.HexColor('#c9c9c9')),
...
def _summary_table(order, styles: dict[str, ParagraphStyle]) -> Table:
    data = [
        ...
        [Paragraph('Total geral', styles['label']), Paragraph(_money(order.total_geral), styles['cell_right_bold']), ''],
    ]
    table = Table(data, colWidths=[70 * mm, 45 * mm, 67 * mm])
    table.setStyle(TableStyle([
        ('SPAN', (0, 0), (-1, 0)),
        ('BACKGROUND', (0, 0), (-1, 0), colors.HexColor('#c51622')),
        ('TEXTCOLOR', (0, 0), (-1, 0), colors.white),
        ('BACKGROUND', (0, 1), (-1, -1), colors.HexColor('#fcfcfc')),
        ('BOX', (0, 0), (-1, -1), 0.7, colors.HexColor('#c9c9c9')),
        ('INNERGRID', (0, 1), (-1, -1), 0.5, colors.HexColor('#dedede')),
        ('ALIGN', (1, 1), (1, -1), 'RIGHT'),
...
def generate_work_order_pdf(order, service_items, part_items) -> bytes:
    ...
    styles: dict[str, ParagraphStyle] = {
        ...
        'section': ParagraphStyle('section', parent=base['Heading2'], fontName='Helvetica-Bold', fontSize=12, leading=15, textColor=colors.HexColor('#c51622'), spaceBefore=2 * mm, spaceAfter=2 * mm),
        ...
        'label': ParagraphStyle('label', parent=base['BodyText'], fontName='Helvetica-Bold', fontSize=9.5, leading=12, textColor=colors.HexColor('#111111')),
        'value': ParagraphStyle('value', parent=base['BodyText'], fontName='Helvetica', fontSize=9.5, leading=12.5, textColor=colors.HexColor('#111111')),
        'muted': ParagraphStyle('muted', parent=base['BodyText'], fontName='Helvetica-Oblique', fontSize=9, leading=12, textColor=colors.HexColor('#666666')),
        'thead': ParagraphStyle('thead', parent=base['BodyText'], fontName='Helvetica-Bold', fontSize=9, leading=11, textColor=colors.white),
        'cell': ParagraphStyle('cell', parent=base['BodyText'], fontName='Helvetica', fontSize=9, leading=11.5, textColor=colors.HexColor('#111111')),
        'cell_center': ParagraphStyle('cell_center', parent=base['BodyText'], fontName='Helvetica', fontSize=9, leading=11.5, alignment=1, textColor=colors.HexColor('#111111')),
        'cell_right': ParagraphStyle('cell_right', parent=base['BodyText'], fontName='Helvetica', fontSize=9, leading=11.5, alignment=TA_RIGHT, textColor=colors.HexColor('#111111')),
        'cell_right_bold': ParagraphStyle('cell_right_bold', parent=base['BodyText'], fontName='Helvetica-Bold', fontSize=9, leading=11.5, alignment=TA_RIGHT, textColor=colors.HexColor('#111111')),
    }
    ...
    header.setStyle(TableStyle([
        ('SPAN', (0, 1), (1, 1)),
        ('BACKGROUND', (0, 0), (-1, -1), colors.HexColor('#c51622')),
        ('BOX', (0, 0), (-1, -1), 0.8, colors.HexColor('#861017')),
    ...
    def draw_footer(canvas, doc):
        ...
        canvas.drawString(doc.leftMargin, 7 * mm, 'ERP Auto Center · ordem de serviço')
